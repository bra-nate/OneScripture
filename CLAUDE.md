...
Server-side proxy for MP3 downloads. Fetches audio from Bible.is CDN and streams it to the browser with correct headers. Required to avoid CORS issues.
...
## Design Tokens
Defined in `tailwind.config.ts`. Do not use arbitrary Tailwind values — extend the config instead.
...
6. **Download always goes through `/api/download` proxy.** Never link directly to Bible.is CDN URLs.
